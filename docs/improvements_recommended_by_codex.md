...
The former Question Paper Submission and Academic Review module has been removed and is no longer supported. Paper-specific portions of the original roadmap are retired; the authentication, authorization, marks-integrity, safe-write, analytics, rate-limit, request-forgery, logging, and safe-error requirements remain applicable.
...
| 1.4 | Stop writes when existing rows cannot be read. Prevent spreadsheet formula interpretation in identifiers and other written values, and enforce payload-size limits. | P0 | `lib/googleSheets.js`, `app/api/marks/route.js` | Medium | 1.3 | High |
...
| 5.3 | Add end-to-end tests for teacher/admin workflows, session expiry, draft isolation/recovery, import validation, and report downloads across mobile and desktop. Include failed network requests and logout/login on shared devices. | P1 | `tests/e2e/` (new), browser-test configuration (new) | Large | 5.1; Phase 4 | Low |
