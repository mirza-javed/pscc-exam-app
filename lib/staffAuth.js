...
export async function getAuthenticatedEmail() {
  ...
  return typeof session?.user?.email === "string" ? session.user.email.trim() : null;
}

export async function getCurrentStaff(authenticatedEmail) {
  const email = authenticatedEmail === undefined
    ? await getAuthenticatedEmail()
    : authenticatedEmail;
  if (!email) return null;
  const authorizationDb = await loadAuthorizationData();
  const staff = findApprovedStaff(authorizationDb.Staff_Directory, email);
