...
import { filterSubjectsForCadet, sortSubjectsWithConductLast } from "./models";
...
export function renderCadetResultCardToDoc(doc, {
  ...
  const tableBody = cadetSubjects.map((subj, idx) => {
...
export function downloadMeritMasterSheetPDF({
  ...
  const finalSubjects = sortSubjectsWithConductLast(subjects);
  ...
  const body = meritGrid.map((c) => {
    ...
    finalSubjects.forEach((subj) => {
      ...
      if (!scoreObj) {
        ...
        row.push(String(scoreObj.obtained));
      }
    });

    row.push(`${c.totalObtained} / ${c.totalMaxMarks}`);
    row.push(`${c.aggregatePct}%`);
  ...
  const foot = [
    [
      ...
      ...finalSubjects.map((subj) => {
        const avgObj = subjectAverages?.find((a) => a.subject === subj);
  ...
  autoTable(doc, {
    ...
    didParseCell: function (data) {
      ...
      if (
        ...
        const subjName = subjects[data.column.index - 4];
        const scoreObj = cadet?.scores?.[subjName];
