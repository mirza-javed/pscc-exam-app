...
import { ALL_EXAMS, getAssessment } from "./examinationResults.mjs";
import {
...
export function downloadMeritMasterSheetPDF({
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    `CLASS: GRADE ${grade}-${section}   •   EXAMINATION: ${exam}   •   ACADEMIC SESSION: ${academicSession || "-"}   •   PAGE SETUP: LEGAL LANDSCAPE`,
    textStartX,
    logoY + 14.5
  );
  ...
  const body = meritGrid.map((c, cadetIndex) => {
    if (isAllExams) {
  ...
  if (!isAllExams) fixedColWidths[3] = { cellWidth: 18, halign: "center" };
