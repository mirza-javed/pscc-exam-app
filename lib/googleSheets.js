...
export async function appendMarksLog(records) {
  ...
  // Format: [Submission_ID, Kit_No, Exam_ID, Subject, Marks_Obtained]
  const rows = records.map((r) => [
    r.Submission_ID || `SUB-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`,
    String(r.Kit_No || r.Student_ID || "").trim(),
    String(r.Exam_ID || "").trim(),
    String(r.Subject || "").trim(),
    String(r.Marks_Obtained !== undefined ? r.Marks_Obtained : "").trim(),
  ]);
  ...
  // Clear cache so subsequent reads see the latest marks
...
export async function appendQuestionPaper(paperData) {
  ...
  const row = [
    paperData.Submission_ID || `QP-${Date.now().toString(36).toUpperCase()}`,
    paperData.Submitted_At || new Date().toISOString(),
    paperData.Teacher_Name || "",
    String(paperData.Grade || ""),
    paperData.Subject || "",
    paperData.Exam_ID || "",
    paperData.Submission_Type || "Direct Text",
    paperData.File_URL || "",
    paperData.Text_Content || "",
    paperData.Status || "Pending",
    paperData.Admin_Feedback || "",
  ];
