...
function summarizeAssessments(assessments) {
  if (!assessments.length) {
    return { state: "NOT_APPLICABLE", obtained: null, maxMarks: null, partialObtained: 0, partialMaxMarks: 0 };
  }
  ...
  for (const assessment of assessments) {
    ...
    else if (["PRESENT", "ABSENT"].includes(assessment.state)) {
      partialObtained += assessment.obtained;
      partialMaxMarks += assessment.maxMarks;
      if (assessment.state === "ABSENT") absentCount++;
    }
  }
  let state = "COMPLETE";
  if (hasConfigurationError) state = "CONFIGURATION_ERROR";
  else if (hasInvalid) state = "INVALID";
